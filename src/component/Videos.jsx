...
const Videos = ()=>{
    ...
    return (
            <Stack direction={['column' ,'row']} minH={"100vh"}>
                <VStack width={'full'}>
                    <video
                    controls controlsList="nodownload" src={videoSrc} style={{
                        width:'100%',
                    }}></video>

                    {/* inside this i am creating one more VStack becuase there i want to give title with some descreption */}

                    <VStack alignItems={"flex-start"} p={"8"} w={"full"} minH={"20vh"}>
                        <Heading>Sample Video</Heading>
                        <Text >Lorem ipsum dolor sit amet consectetur adipisicing elit. A maxime voluptas perferendis voluptates est nostrum sapiente, illum culpa in neque rem provident asperiores quibusdam dolore quod soluta atque cum amet.</Text>
                    </VStack>

                </VStack>

                {/* here we are creating second vStack because in side we want to print videos */} 

                <VStack w={['full','xl']} alignItems="stretch" p="8" spacing={'8'} overflowY="auto" >

                   {/* here i am going to use map array method to print all the lectures value inside the buttom */}
                   {videoArr.map((item,index)=>{
                    return  <Button variant={"ghost"} colorScheme="purple" onClick={()=>setVideoSrc(item)} > Video {index + 1} </Button>
                   })}
                </VStack>
            </Stack>
    )
